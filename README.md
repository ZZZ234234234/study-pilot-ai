...
**当前桌面版本：v1.0.1。** Windows 安装包只有在自动化检查、生产构建、实际安装与重启验证全部通过后才会进入 GitHub Releases。真实付费模型的回答质量、全部 Windows 设备和长期使用稳定性仍需持续验证；这是本地优先的开源应用，不是托管在线服务。

项目由嘉兴大学通信专业学生 **“爱吃孜然芥末”** 独立制作；该身份说明不代表嘉兴大学官方开发、授权或背书。使用问题请联系 `2014546082@qq.com`。

[快速开始](#快速开始) · [API 接入](#api-接入deepseek--智谱) · [阅读布局](#阅读布局与悬浮助手) · [论文翻译](#pdf-中英对照翻译) · [格式转换](#本地格式转换) · [中英文切换](#中英文切换) · [配置模型](#demo-与真实模型) · [验证状态](#当前验证状态) · [MIT](LICENSE)
...
| 问答     | 可切换 DeepSeek / 智谱，跨语言检索词 + 本地关键词检索、原文引用；保留原有 pgvector 模式 |
...
下载 `StudyPilot-AI-1.0.1-Windows-x64-Setup.exe` 后安装，通过桌面快捷方式启动，不需要另装 Node、Python 或数据库。AI 问答和翻译仍需在应用的「API 接入」中填写自己的 DeepSeek / 智谱配置。
...
```powershell
certutil -hashfile "StudyPilot-AI-1.0.1-Windows-x64-Setup.exe" SHA256
gh attestation verify "StudyPilot-AI-1.0.1-Windows-x64-Setup.exe" -R ZZZ234234234/study-pilot-ai
```
...
2. 选择 **DeepSeek** 或 **智谱**，接口地址自动填入官方通用 API 地址。
3. 填写连接备注名、自己的 **API Key** 和 **准确模型 ID**。同一服务商的不同型号可以分别保存，最多 12 个连接。
4. DeepSeek 可点击 **读取官方模型列表**。智谱提供官方文档参考型号和手动输入；参考列表不代表账号已经有权限，不会根据模糊名字猜测型号。模型以服务商当前实际提供为准。
...
第一版仅支持官方通用端点：DeepSeek `https://api.deepseek.com/v1`，智谱 `https://open.bigmodel.cn/api/paas/v4`。不支持自定义中转站或 Coding Plan 专用端点。新适配器不跟随重定向、不读取系统代理配置、不自动重试收费请求。需要网络代理的环境须另行评估部署方案。

“请求内上下文记忆”只表示模型能使用本次检测请求中提供的较早消息；“临时示例学习”只表示模型能在上下文内按新规则回答，**不等于训练、永久学习或形成用户画像**。本项目不实现跨对话长期记忆。

接口依据：[DeepSeek 模型列表](https://api-docs.deepseek.com/api/list-models/)、[DeepSeek JSON 输出](https://api-docs.deepseek.com/guides/json_mode/)、[智谱兼容接口](https://docs.bigmodel.cn/cn/guide/develop/openai/introduction)。参考型号核对日期：2026-09-02；参考不代表免费额度、模型可用性或真实联调已通过。
...
1. 在 **API 接入** 中保存 DeepSeek / 智谱，并在翻译面板选择型号；也可选择原有 `.env` 配置的 `CHAT_MODEL`。要求支持 JSON 输出；Demo 不提供假译文。
...
| 验证范围                 | 结果                                                                                        |
| ------------------------ | ------------------------------------------------------------------------------------------- |
| 前端逻辑、组件与转换测试 | 110 项通过；含模型表单、切换授权、阅读布局、双语与转换测试                                  |
| 后端单元与接口测试       | 70 项通过；含私有模型连接、迁移、跨语言检索及翻译协议测试                                   |
| 接口端到端测试           | 20 项通过，使用生产前端构建和独立临时服务                                                   |
...
**合计 200 项自动测试通过，不包含真实浏览器 UI 测试。** 实际浏览器中的全屏阅读、悬浮窗拖动、翻译操作、文件下载和移动端布局，以及真实 DeepSeek / 智谱调用和论文译文质量，仍需进一步验证。Windows/macOS 的全新安装和生产部署也尚未完成环境验收；上方启动命令不代表这些平台已经全部实测。完整范围与限制见[验证记录](docs/verification.md)。
