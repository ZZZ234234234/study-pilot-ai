...
[快速开始](#快速开始) · [阅读布局](#阅读布局与悬浮助手) · [论文翻译](#pdf-中英对照翻译) · [格式转换](#本地格式转换) · [中英文切换](#中英文切换) · [配置模型](#demo-与真实模型) · [验证状态](#当前验证状态) · [MIT](LICENSE)
...
页面包括产品首页、学习仪表盘、文档库、PDF 学习工作区、复习计划、学习工具箱、设置、隐私说明和开源说明。内置样例可以演示阅读、问答、计划、闪卡和测验的完整流程；它使用固定内容，不代表真实模型的生成效果，也不会模拟 AI 翻译。
...
- **收起导航**：左上角按钮可展开或收起侧边导航，首次访问默认收起。桌面选择会保存在当前浏览器；手机使用抽屉菜单，点击遮罩、关闭按钮或按 Esc 退出。
...
如果最初下载的是 ZIP，请将新版解压到新文件夹，不要直接覆盖旧目录。保留旧目录的 `.env`、`data/` 和浏览器资料；按上面的安装步骤准备新版，在服务停止时迁移自己的配置与数据，确认新版正常后再考虑处理旧目录。自定义 `DATA_DIR`、数据库地址或存储路径的部署需继续使用原配置。本次翻译与工具箱更新没有修改数据库结构，不会清空已有资料；新增了前端转换依赖，更新后请运行 `npm ci`。
...
1. 按上面的模型说明配置真实聊天模型并重启服务。翻译复用 `CHAT_MODEL`，要求模型支持 JSON 输出；Demo 不提供假译文。
...
同一阅读会话中，页码、语言、风格、术语相同的已完成结果会复用；更换选项会重新请求模型。点击 **停止后续页** 会等待当前页完成，不再发送下一页；已经发出的请求可能继续计费。某页失败时保留此前完成的结果，重试会跳过已完成页。
...
| 验证范围                 | 结果                                                                                        |
| ------------------------ | ------------------------------------------------------------------------------------------- |
| 前端逻辑、组件与转换测试 | 103 项通过；包括阅读布局、jsdom 组件交互与 Node canvas 编码测试，不等于真实浏览器           |
| 后端单元与翻译接口测试   | 45 项通过；翻译使用协议测试替身，不是真实模型质量评测                                       |
| 接口端到端测试           | 18 项通过，使用生产前端构建和独立临时服务                                                   |
...
**合计 166 项自动测试通过，不包含真实浏览器 UI 测试。** 实际浏览器中的全屏阅读、悬浮窗拖动、翻译操作、文件下载和移动端布局，以及真实聊天/嵌入模型调用和论文译文质量，仍需进一步验证。Windows/macOS 的全新安装和生产部署也尚未完成环境验收；上方启动命令不代表这些平台已经全部实测。完整范围与限制见[验证记录](docs/verification.md)。
