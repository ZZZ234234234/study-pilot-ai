...
[快速开始](#快速开始) · [第一次使用](#第一次使用) · [论文翻译](#pdf-中英对照翻译) · [格式转换](#本地格式转换) · [中英文切换](#中英文切换) · [配置模型](#demo-与真实模型) · [验证状态](#当前验证状态) · [MIT](LICENSE)
...
| 验证范围                 | 结果                                                                                        |
| ------------------------ | ------------------------------------------------------------------------------------------- |
| 前端逻辑、组件与转换测试 | 68 项通过；包括 jsdom 组件交互与 Node canvas 编码测试，不等于真实浏览器                     |
...
**合计 131 项自动测试通过，不包含真实浏览器 UI 测试。** 实际浏览器中的翻译操作、文件下载、刷新保留和移动端布局，以及真实聊天/嵌入模型调用和论文译文质量，仍需进一步验证。Windows/macOS 的全新安装和生产部署也尚未完成环境验收；上方启动命令不代表这些平台已经全部实测。完整范围与限制见[验证记录](docs/verification.md)。
