...
`/app/models` manages `/ai/profiles`. A profile has an owner, nickname, provider, allowlisted canonical endpoint, exact model ID, private key and revision. `User.ai_profile_id` is the workspace default. Writes lock the owning user row, limit connections to twelve and check ownership. Public serializers explicitly enumerate safe fields; do not use `row_dict` for `AIProfile`. Keys are currently unencrypted in the backend database; protect the local `data/` directory, database and backups. User-entered keys travel through the same-origin proxy to the backend once, not to browser storage or public code.
...
Only official general-purpose DeepSeek and Zhipu HTTPS endpoints are accepted. The adapter does not follow redirects, read environment proxies or automatically retry billable calls. API errors exclude upstream bodies and credentials. DeepSeek can list model IDs from its official `/models` API. Zhipu uses a dated documentation reference catalog/manual ID entry; it does not claim to discover account access. A user-triggered connection test checks the draft's exact model and JSON output with a 64-output-token cap and no PDF text. Saving and testing are separate actions.
